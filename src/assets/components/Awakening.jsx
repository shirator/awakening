...
export default function Awakening() {
  ...
  function regrade() {
    let tap = rng()
    let current_chance = chance + ((attempt + 1) * 5)
  ...
  return (
    <div className="flex flex-col gap-5 items-center border-2 rounded-md p-10 min-h-8">
      ...
      <div className="flex flex-col gap-2 items-start text-xl w-72">
        ...
        {attempt > 0 ? (success ? "gratz you got t2!" : "get better luck") : ""}
      </p>
      </div>
      <button className={`w-full uppercase ${success ? 'bg-red-500 cursor-not-allowed' : ''}`} onClick={regrade} disabled={success}>Awake</button>
      <button onClick={() => {
        setSuccess(false)
        setAttempt(0)
      }}
      className="w-full uppercase">restart</button>
    </div>
  );
}
